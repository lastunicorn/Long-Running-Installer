...
namespace DustInTheWind.LongRunningInstaller.Cli
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            log.WriteInfo(new Title("Begin LongRun"));

            LongRunRequest request = new LongRunRequest
            {
                Seconds = 5,
                StopDir = Environment.CurrentDirectory
            };

            LongRunUseCase useCase = new LongRunUseCase(log);
            useCase.Execute(request);

            log.WriteInfo(new Title("End LongRun"));
        }
    }
}